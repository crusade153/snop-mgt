...
- ⚠️ **주중에 미리 적재해 둔 재고는 마감 재고로 교체된다**(`staleMidWeekCapture`, `lib/weekly-snapshot.ts`).
  ...
  **재고가 주중 값으로 영영 굳었다.** 그래서 기존 `created_at` 이 **주 마감(일요일 KST 자정 = weekEnd 15:00 UTC)
  이전**이면 재고를 다시 쓴다. 반대로 **마감 후에 찍힌 재고는 절대 덮지 않는다** — 소급 불가라
  다시 찍으면 「그때의 재고」가 아니라 「지금 재고」가 들어온다. 이 두 경우를 뭉개지 말 것.
