...
- **주간 요약장표(`/weekly`)의 판정 기준** — `lib/weekly/classification.ts` 하나에 모여 있다.
  ...
  **후보 순서(플랜트 코드 오름차순)는 뒤집지 말 것** — 분류되는 코드가 둘인 12품목(`1021:A08` + `1022:M11` 등)이 예전 값을 유지해야 한다.
  ⚠️ 남은 미매핑은 **마스터정비 5.15억(4.0%)** 뿐이고 대부분 1031 전용 영업 코드(M33 2.98억·M36 1.68억)다. 기준정보 정비 중이라 의도적으로 기타에 둔다.
  (26.5% → 13.9%(A 계열 흡수) → 6.1%(복수 DISPO 대표값 교정) → 4.0%(18=즉석밥 + H01 폴백) 순으로 줄었다.)
