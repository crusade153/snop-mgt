        ...
        <div className="overflow-x-auto min-h-[500px]">
          <table className="w-full text-sm text-left border-collapse">
            ...
            <tbody className="divide-y divide-neutral-200">
              {paginatedItems.map((item: IntegratedItem, idx: number) => {
                ...
                return (
                  <tr key={item.code} className="hover:bg-[#F9F9F9] transition-colors h-[48px]">
                    ...
                    {/* 🚨 [수정] 단위 컬럼도 동적으로 표시 */}
                    <td className="px-4 py-3 text-center text-neutral-500">{displayStock.unit}</td>
                    
                    <td className="px-4 py-3 text-right font-bold text-neutral-800">
                      {displayStock.value}
                    </td>
                    <td className="px-4 py-3 text-center text-neutral-600">
                      <div className="flex items-center justify-center gap-1"><Calendar size={12} className="text-neutral-400" />{expiryDate}</div>
                    </td>
                    <td className={`px-4 py-3 text-right ${daysColor}`}>{item.inventory.remainingDays}일</td>
              ...
              {paginatedItems.length === 0 && (<tr><td colSpan={9} className="p-10 text-center text-neutral-400">검색된 재고가 없습니다.</td></tr>)}
            </tbody>
          </table>
        </div>
...
function TabButton({ label, count, active, onClick, color }: any) {
  return (
    <button onClick={onClick} className={`flex items-center gap-2 px-3 py-2 rounded-md text-xs font-bold transition-all ${active ? 'bg-white shadow-sm text-neutral-900' : 'text-neutral-500 hover:text-neutral-700'}`}>
