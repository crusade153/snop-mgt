...
export default function Sidebar() {
  ...
  return (
<aside 
      // width를 style 변수 대신 Tailwind 클래스(w-[240px])로 고정
      className="fixed left-0 top-0 bottom-0 z-50 flex flex-col w-[240px] bg-[#FAFAFA] border-r border-neutral-200"
    >
      {/* Brand Identity: 하림 로고 */}
      <div className="h-[60px] flex items-center px-6 border-b border-neutral-200 bg-white">
        <div className="flex items-center gap-2">
          {/* 하림 로고 (원형 + 텍스트 조합 시뮬레이션) */}
        ...
        {menuItems.map((item) => {
          ...
          return (
            <Link
              ...
              href={item.href}
              className={`
                group flex items-center justify-between px-3 py-2.5 rounded text-sm font-medium transition-all duration-150 ease-in-out
                ${isActive 
                  ? 'bg-[#E3F2FD] text-primary-blue border-l-[3px] border-primary-blue' 
                  : 'text-neutral-700 hover:bg-neutral-200 border-l-[3px] border-transparent'
                }
              `}
            >
              ...
              {/* Active 시 Chevron 표시 (Interaction Pattern) */}
      ...
      <div className="p-4 border-t border-neutral-200 bg-neutral-50">
        <div className="text-[10px] text-neutral-500 text-center">
          © 2026 Harim Holdings.<br/>All rights reserved.
        </div>
      </div>
    </aside>
  );
